...
describe("DollarFlipt", () => {
  describe("evaluate method", () => {
    it("should throw an error if no entityId is provided", async () => {
      expect.assertions(1);
      const df = new DollarFlipt({ baseURL: "https://myserver.io" });
      try {
        await df.evaluate();
      } catch (e) {
    ...
    describe("when doing api calls", () => {
      ...
      it("should hit the evaluate api with a custom baseURL", async () => {
        mockAddToCache.mockImplementation();
        mockGetCached.mockImplementation();
        const df = new DollarFlipt();

        const request = await df.evaluate(
          "entity-1",
          "test-flag",
          {
            value1: "red",
      ...
      it("should throw an error if the request fails", async () => {
        const df = new DollarFlipt({ baseURL: "https://mywrongserver.io" });
